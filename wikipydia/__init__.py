...
def get_all_categories( aclimit=None, language='en'):
    """ """
    query_args = {
        'action': 'query',
        'list':'allcategories',
    ...
    if aclimit is not None:
        query_args.update({'aclimit':aclimit})

    while query_args['acfrom'] != '':
        json = _run_query(query_args, language)

        response = {
            'categories': json['query']['allcategories'],
            'acfrom': json['query-continue']['allcategories']['acfrom'],
        }

        query_args['acfrom'] = response['acfrom']
        yield [category['*'] for category in response['categories']]
